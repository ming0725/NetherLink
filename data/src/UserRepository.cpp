...
User UserRepository::getUser(const QString& userID) {
    ...
    QSqlQuery query(db);
    query.prepare("SELECT u.id, u.name, f.remark, u.avatar_url, f.status, u.signature "
                  "FROM users u LEFT JOIN friends f ON u.uid = f.friend_id "
                  "WHERE u.id = ?");
...
QVector<User> UserRepository::getAllUser() {
    ...
    if (query.exec("SELECT u.id, u.name, f.remark, u.avatar_url, f.status, u.signature "
                  "FROM users u LEFT JOIN friends f ON u.uid = f.friend_id")) {
        while (query.next()) {
            User user;
            user.id = query.value(0).toString();
...
void UserRepository::insertUser(const User& user) {
    ...
    userQuery.prepare("INSERT OR REPLACE INTO users (uid, id, name, avatar_url, signature) "
                     "VALUES (?, ?, ?, ?, ?)");
    ...
    if (success) {
        ...
        friendQuery.prepare("INSERT OR REPLACE INTO friends (friend_id, status, remark, group_name) "
                          "VALUES (?, ?, ?, ?)");
...
bool UserRepository::updateCurrentUser(const QJsonObject& userInfo) {
    ...
    // 更新当前用户信息
    QSqlQuery userQuery(db);
    userQuery.prepare("INSERT OR REPLACE INTO users (uid, id, name, avatar_url) "
                     "VALUES (?, ?, ?, ?)");

...
Group GroupRepository::getGroup(const QString& groupID) {
    ...
    // 获取群组基本信息
    query.prepare(
        "SELECT cg.gid, cg.name, "
        "(SELECT COUNT(*) FROM group_members WHERE gid = cg.gid) as member_count, "
        "cg.owner_id, cg.avatar_url, cg.remark, cg.announcement "
        "FROM chat_groups cg "
        "WHERE cg.gid = ?"
    );
    ...
    if (query.exec() && query.next()) {
        ...
        memberQuery.prepare(
            "SELECT uid, name, avatar_url, nickname, role "
            "FROM group_members "
            "WHERE gid = ?"
        );
...
void GroupRepository::insertGroup(const Group& group) {
    ...
    groupQuery.prepare(
        ...
        "(gid, name, owner_id, avatar_url, remark) "
        "VALUES (?, ?, ?, ?, ?)"
    );
    ...
    if (success && !group.members.isEmpty()) {
        ...
        memberQuery.prepare(
            ...
            "(gid, uid, name, avatar_url, nickname, role) "
            "VALUES (?, ?, ?, ?, ?, ?)"
        );

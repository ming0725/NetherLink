...
- 当前只覆盖已有通知页的同意/拒绝按钮、好友搜索/群搜索发起申请、好友资料编辑和删除好友入口，已有创建群、群资料编辑、群成员管理、转让群主、我的群设置和退群入口，会话置顶/免打扰/隐藏/已读/未读/清空入口，以及消息撤回入口。

### 消息历史分页补拉

- `ChatMessage` 已保留服务端 `messageSeq` 并写入 `chat_messages` 本地快照。
- 首次打开会话仍先使用本地最近消息快照；当最早已加载消息带有 `messageSeq > 1` 且达到页大小时，`ChatArea` 保留“可加载更早消息”的状态。
- 用户上滑触发加载更早消息时，`MessageRepository` 会在本地缓存边界处请求 `GET /api/v1/conversations/{conversationId}/messages?beforeMessageSeq=<oldestSeq>&limit=<pageSize>`。
- 服务端返回的历史消息复用 `cacheRemoteMessageObject()` 写入 `chat_messages`，再按现有 prepend 流程插入当前聊天窗口。
- 补拉失败或服务端返回空页时，不构造本地历史消息；UI 只收敛为没有更多历史或保持已有缓存。

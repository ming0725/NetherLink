...
- 好友申请和群入群申请的同意/拒绝按钮已经先调用远程 API，成功后再更新 `friend_notifications`、`group_notifications`、`users`、`groups`、`conversations` 等本地快照；删除好友也先调用远程 API，成功后再移除本地会话和好友缓存；失败只显示错误提示，不把静态样例或纯本地结果当作后端确认。
...
- 好友申请和群入群申请同意/拒绝由 `FriendRemoteDataSource` 先发送远程请求，成功后复用现有 repository 更新本地通知、好友/群成员和会话提示消息；删除好友同样由 `FriendRemoteDataSource` 远程确认后清理 `users` 和 `conversations`；暂未引入通用 `sync_outbox` 表。

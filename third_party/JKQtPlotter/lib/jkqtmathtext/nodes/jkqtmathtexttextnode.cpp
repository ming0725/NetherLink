...
bool isMathItalicCodepoint(uint32_t codepoint)
{
    return codepoint==0x210E || (codepoint>=0x1D434 && codepoint<=0x1D467);
}

uint32_t mathItalicCodepoint(QChar c)
{
    if (c>='A' && c<='Z') return 0x1D434 + c.unicode() - QChar('A').unicode();
    if (c>='a' && c<='z') {
        if (c=='h') return 0x210E;
        return 0x1D44E + c.unicode() - QChar('a').unicode();
    }
    return 0;
}

uint32_t mathScriptCodepoint(QChar c, bool bold)
{
    if (bold) {
        if (c>='A' && c<='Z') return 0x1D4D0 + c.unicode() - QChar('A').unicode();
        if (c>='a' && c<='z') return 0x1D4EA + c.unicode() - QChar('a').unicode();
        return 0;
    }

    switch (c.unicode()) {
        case 'A': return 0x1D49C;
        case 'B': return 0x212C;
        case 'C': return 0x1D49E;
        case 'D': return 0x1D49F;
        case 'E': return 0x2130;
        case 'F': return 0x2131;
        case 'G': return 0x1D4A2;
        case 'H': return 0x210B;
        case 'I': return 0x2110;
        case 'J': return 0x1D4A5;
        case 'K': return 0x1D4A6;
        case 'L': return 0x2112;
        case 'M': return 0x2133;
        case 'N': return 0x1D4A9;
        case 'O': return 0x1D4AA;
        case 'P': return 0x1D4AB;
        case 'Q': return 0x1D4AC;
        case 'R': return 0x211B;
        case 'S': return 0x1D4AE;
        case 'T': return 0x1D4AF;
        case 'U': return 0x1D4B0;
        case 'V': return 0x1D4B1;
        case 'W': return 0x1D4B2;
        case 'X': return 0x1D4B3;
        case 'Y': return 0x1D4B4;
        case 'Z': return 0x1D4B5;
        case 'a': return 0x1D4B6;
        case 'b': return 0x1D4B7;
        case 'c': return 0x1D4B8;
        case 'd': return 0x1D4B9;
        case 'e': return 0x212F;
        case 'f': return 0x1D4BB;
        case 'g': return 0x210A;
        case 'h': return 0x1D4BD;
        case 'i': return 0x1D4BE;
        case 'j': return 0x1D4BF;
        case 'k': return 0x1D4C0;
        case 'l': return 0x1D4C1;
        case 'm': return 0x1D4C2;
        case 'n': return 0x1D4C3;
        case 'o': return 0x2134;
        case 'p': return 0x1D4C5;
        case 'q': return 0x1D4C6;
        case 'r': return 0x1D4C7;
        case 's': return 0x1D4C8;
        case 't': return 0x1D4C9;
        case 'u': return 0x1D4CA;
        case 'v': return 0x1D4CB;
        case 'w': return 0x1D4CC;
        case 'x': return 0x1D4CD;
        case 'y': return 0x1D4CE;
        case 'z': return 0x1D4CF;
        default: return 0;
    }
}
...
void JKQTMathTextTextNode::splitTextForLayout(QPainter &painter, JKQTMathTextEnvironment currentEv, const QString &txt, QStringList &textpart, QList<FontMode> &fontMode) const
{
    ...
    while (i<txt.size()) {
        ...
        uint32_t codepoint=c.unicode();
        if (c.isHighSurrogate() && i+1<txt.size() && txt[i+1].isLowSurrogate()) {
            codepoint=QChar::surrogateToUcs4(c, txt[i+1]);
            cs+=txt[i+1];
            i++;
        }
        const bool CisForcedUprightChar=isForcedUprightChar(c);
        const bool CisForcedUprightCharExt=CisForcedUprightChar||(c=='.')||(c==',');
        if (currentEv.insideMath && isMathItalicCodepoint(codepoint)) {
            CFontMode=FMasDefinedForceUpright;
        } else if (currentEv.insideMath && currentEv.insideMathForceDigitsUpright && (currentEv.font==MTEroman || currentEv.font==MTEmathRoman)) {
        ...
        if (currentEv.insideMath) {
            ...
            for (int i=0; i<text.size(); i++) {
                ...
                switch(c.unicode()) {
                    ...
                    case ',': txt+=QString(QString(", ")); break;
                    default: {
                        const uint32_t scriptCodepoint = currentEv.font==MTEcaligraphic ? mathScriptCodepoint(c, currentEv.bold) : 0;
                        const uint32_t italicCodepoint = currentEv.italic ? mathItalicCodepoint(c) : 0;
                        if (scriptCodepoint!=0) txt+=jkqtp_UnicodeToUTF8Q(scriptCodepoint);
                        else if (italicCodepoint!=0) txt+=jkqtp_UnicodeToUTF8Q(italicCodepoint);
                        else txt+=c;
                    } break;
                }
            }

...
double JKQTMathTextBraceNode::draw(QPainter& painter, double x, double y, JKQTMathTextEnvironment currentEv) const {
    ...
    const double absnorm_linewidth_factor=0.60;
    const double absnorm_doubleline_offset_factor=1.35;
    ...
    const double angle_centerwidth=lw*1.55;
    {
        ...
        if (openbrace==MTBTParenthesis) {
            ...
            plocal.setWidthF(plocal.widthF()*absnorm_linewidth_factor);
            painter.setPen(plocal);
            const QLineF l(xbraceC-lw*absnorm_doubleline_offset_factor, y1, xbraceC-lw*absnorm_doubleline_offset_factor, y2);
            if (l.length()>0) painter.drawLine(l);
            const QLineF l2(xbraceC+lw*absnorm_doubleline_offset_factor, y1, xbraceC+lw*absnorm_doubleline_offset_factor, y2);
...
QSizeF JKQTMathTextBraceNode::calcBraceSize(const QFont &f, QPaintDevice *pd, JKQTMathTextBraceType bracetype, const JKQTMathTextNodeSize &childSize) const
{
    ...
    const double dblline_distance=2.7*lw;
    ...
    if (bracetype==MTBTDoubleLine)  braceWidth=dblline_distance+3.2*lw;

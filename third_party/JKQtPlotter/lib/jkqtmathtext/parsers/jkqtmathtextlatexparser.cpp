/*
    ...
    Changes: added single-token math argument parsing for fraction-style
    commands; see third_party/JKQtPlotter/MODIFICATIONS.md.
...
JKQTMathTextNode* JKQTMathTextLatexParser::parseMathArgument(bool get, bool *foundError, bool* getNew) {
    ...
    JKQTMathTextNode* child=nullptr;
    if (currentToken==MTTinstruction) {
        child=parseInstruction(foundError, getNew);
    } else if (currentToken==MTTopenbrace) {
        child=parseLatexString(true);
        if (getNew) *getNew=true;
    } else if (currentToken==MTTtext) {
        ...
        child=new JKQTMathTextTextNode(parentMathText, "[", false, parsingMathEnvironment);
        if (getNew) *getNew=true;
    } else if (currentToken==MTTclosebracket) {
        child=new JKQTMathTextTextNode(parentMathText, "]", false, parsingMathEnvironment);
        if (getNew) *getNew=true;
...
JKQTMathTextNode* JKQTMathTextLatexParser::parseInstruction(bool *_foundError, bool* getNew) {
    ...
    if (JKQTMathTextWhitespaceNode::supportsInstructionName(currentInstructionName)) {
        ...
        bool numeratorError=false;
        bool denominatorError=false;
        bool getNewAfterNumerator=true;
        bool getNewAfterDenominator=true;
        JKQTMathTextNode* n1=parseMathArgument(true, &numeratorError, &getNewAfterNumerator);
        JKQTMathTextNode* n2=parseMathArgument(getNewAfterNumerator, &denominatorError, &getNewAfterDenominator);
        if (getNew) *getNew=getNewAfterDenominator;
        if (!numeratorError && !denominatorError && n1 && n2) {
            child=new JKQTMathTextFracNode(parentMathText, n1, n2, JKQTMathTextFracNode::InstructionName2FracType(currentInstructionName));
        } else {
            if (n1) delete n1;
            if (n2) delete n2;
            addToErrorList(tr("error @ ch. %1: expected two arguments after '%2' command").arg(currentTokenID).arg(currentInstructionName));
        }

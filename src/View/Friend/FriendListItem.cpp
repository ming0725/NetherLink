...
#include "View/Friend/FriendListItem.h"
...
FriendListItem::FriendListItem(const User& user, QWidget* parent) : QWidget(parent), avatarLabel(new AvatarLabel(this)), fullNameText(user.nick), status(user.status), userID(user.id) {
    ...
    avatarLabel->loadAvatar(user.id, user.avatarPath, false);
}
...
void FriendListItem::setupUI(const User& user) {
    ...
    statusIconLabel->setPixmap(QPixmap(statusIconPath(user.status)).scaled(statusIconSize, statusIconSize, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
void FriendListItem::resizeEvent(QResizeEvent* event) {
    ...
    // 1. 头像：垂直居中
    avatarLabel->setGeometry(leftPad, cy - avatarSize / 2, avatarSize, avatarSize);
    ...
    // 4. 姓名和时间戳底部对齐
    nameLabel->setGeometry(contentX, nameY_top, w - contentX - leftPad - spacing, nameH);
    {
    ...
    statusTextLabel->setGeometry(contentX + statusIconLabel->width() + icoTextDist, textY_top + 1, textW_available, textH);
    {

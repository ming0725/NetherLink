...
/* function --------------------------------------------------------------- 80 // ! ----------------------------- 120 */
    ...
    ToastTip& ToastTip::instance() {
        static ToastTip mgr;

        return (mgr);
    }

    // void ToastTip::setupUI() {
    ...
    // ui->iconLabel = new QLabel(this);

    // ui->iconLabel->setFixedSize(20, 20);

    // ui->iconLabel->setScaledContents(true);

    //// layout->addWidget(ui->iconLabel);

    // ui->messageLabel = new QLabel(this);

    // ui->messageLabel->setStyleSheet("QLabel { color: white; }");

    //// layout->addWidget(ui->messageLabel);
    ...
    /* 全屏顶部居中版本（不变动） */
    void ToastTip::showMessage(const QString& message, Type type) {
...
// 3s 后自动隐藏
    ...
    /***新增：在指定 QWidget 顶部弹出版本 */ void ToastTip::showMessage(const QString& message, Type type, QWidget* targetWidget) {
...
// 保持高度固定 40px，但宽度根据文字长度 + 图标 + 左右 margin/spacing 自动调整
        QFontMetrics fm(ui->messageLabel->font());
        int textWidth = fm.horizontalAdvance(message);
        int iconW = ui->iconLabel->width();   // 通常为 20px

...
CustomPushButton::CustomPushButton(QWidget* parent)
    : QPushButton(parent)
    , m_radius(20)
    , m_backgroundColor(QColor(0x0099ff))
    , m_hoverColor(QColor(0x0088ee))
    , m_pressColor(QColor(0x0077dd))
...
void CustomPushButton::updateStyle()
{
    if (m_backgroundAnimation && m_backgroundAnimation->state() != QAbstractAnimation::Running) {
        m_backgroundColor = m_isPressed ? m_pressColor : 
                          m_isHovered ? m_hoverColor : 
                          property("backgroundColor").value<QColor>();
    }
    update();
}
...
QColor CustomPushButton::backgroundColor() const { return m_backgroundColor; }
...
void CustomPushButton::setCustomStyle(const QColor& background, const QColor& hover, 
                               const QColor& press, const QColor& text)
{

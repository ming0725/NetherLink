    ...
    if (!m_selectedImagePath.isEmpty()) {
        ...
        if (file->open(QIODevice::ReadOnly)) {
            ...
            imagePart.setHeader(QNetworkRequest::ContentDispositionHeader,
                                QVariant("form-data; name=\"images\"; filename=\"" + fileName + "\""));
            imagePart.setHeader(QNetworkRequest::ContentTypeHeader,
                                QVariant(mimeTypeForFile(info.filePath())));
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (doc.isObject() && doc.object()["code"].toInt() == 0) {
                ...
                NotificationManager::instance()
                        .showMessage(tr("发布失败: %1").arg(obj.value("message").toString()),
                                     NotificationManager::Error,
                                     MainWindow::getInstance());
            }
        } else {
            NotificationManager::instance()
                    .showMessage(tr("网络错误: %1").arg(reply->errorString()),
                                 NotificationManager::Error,
                                 MainWindow::getInstance());
        }
        reply->deleteLater();

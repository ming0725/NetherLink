...
SearchResultItem::SearchResultItem(const User& user, QString& uid, QWidget* parent)
    : QWidget(parent)
...
SearchResultItem::SearchResultItem(const QString& groupId,
...
void SearchResultItem::setupUI()
{
    ...
    if (type == UserType) {
        statusIconLabel->setPixmap(QPixmap(::statusIconPath(status))
            .scaled(STATUS_ICON_SIZE, STATUS_ICON_SIZE, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    } else {
        statusIconLabel->setPixmap(QPixmap(":/resources/icon/friend_selected.png")
            .scaled(STATUS_ICON_SIZE, STATUS_ICON_SIZE, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    }
...
{
    auto* window = new ApplyWindow(
        type == UserType ? ApplyWindow::User : ApplyWindow::Group,
        id,
        name,
        avatarUrl
    );
...
void SearchResultItem::resizeEvent(QResizeEvent* event)
{
    ...
    nameLabel->setGeometry(
        contentX,
        nameY_top,
        w - contentX - LEFT_PADDING - BUTTON_WIDTH - SPACING,
        nameH
    );

    // 按钮位置
    actionButton->setGeometry(
        w - LEFT_PADDING - BUTTON_WIDTH,
        cy - BUTTON_HEIGHT / 2,
        BUTTON_WIDTH,
        BUTTON_HEIGHT
    );

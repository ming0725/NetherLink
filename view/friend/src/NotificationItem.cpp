...
NotificationItem::NotificationItem(const QString& text, QWidget* parent)
    : QWidget(parent)
    ...
    , badge(new NotificationBadge(this))
{
    ...
    // 设置箭头图标
    QPixmap arrowPixmap(":/resources/icon/right_arrow.png");
...
void NotificationItem::resizeEvent(QResizeEvent *event) {
    ...
    badge->setGeometry(width() - padding - arrowWidth - badgeWidth - 8,
                      badgeY + 2,
                      badgeSize.width(),
                      badgeSize.height());
    
    // 箭头位置
    arrowLabel->setGeometry(width() - padding - arrowWidth, 
                           (height() - 16) / 2,
                           arrowWidth,
                           16);
}

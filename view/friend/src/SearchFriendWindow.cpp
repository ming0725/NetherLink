...
SearchFriendWindow::SearchFriendWindow(QWidget* parent)
...
void SearchFriendWindow::resizeEvent(QResizeEvent* event)
{
    ...
    searchBox->setGeometry((w - searchBoxWidth) / 2, 
                          TITLE_HEIGHT + MARGIN,
                          searchBoxWidth,
                          SEARCH_BOX_HEIGHT);
    
    // 设置类型选项卡位置
    typeTab->setGeometry(0, 
                        TITLE_HEIGHT + MARGIN + SEARCH_BOX_HEIGHT + MARGIN/2,
                        w,
                        typeTab->height());

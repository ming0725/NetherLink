        ...
        , isGroupMode(false)
{
    ...
    // 连接MessageHandler信号
    connect(&MessageHandler::instance(), &MessageHandler::messageReceived,
            this, &ChatArea::onMessageReceived);
...
void ChatArea::onSendText(const QString& text)
{
    ...
    auto message = QSharedPointer<TextMessage>(new TextMessage(
        text, true, currentUserId, isGroupMode, CurrentUser::instance().getUserName(), role
    ));

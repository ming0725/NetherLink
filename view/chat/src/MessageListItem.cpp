...
MessageListItem::MessageListItem(const MessageDataPtr& messageData, QWidget* parent)
        : QWidget(parent)
...
void MessageListItem::paintEvent(QPaintEvent*) {
    ...
    p.drawText(nameRect, Qt::AlignLeft|Qt::AlignVCenter,
               QFontMetrics(p.font()).elidedText(data->senderName, Qt::ElideRight, nameRect.width()));
    ...
    p.drawText(textRect, Qt::AlignLeft|Qt::AlignVCenter,
               QFontMetrics(p.font()).elidedText(data->content, Qt::ElideRight, textRect.width()));
}

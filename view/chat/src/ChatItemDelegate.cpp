...
{
    // 连接头像加载完成信号
    connect(&AvatarLoader::instance(), &AvatarLoader::avatarLoadStatusChanged,
            this, [this](AvatarLoadEvent* event) {
                                     ...
                                     const ChatMessage* message) const
{
    ...
    // 添加复制选项
    if (message->getType() == MessageType::Text) {
        QIcon copyIcon(":/resources/icon/copy.png");
        ...
        connect(copyAction, &QAction::triggered, [message, index, model = const_cast<QAbstractItemModel*>(index.model())]() {
            ...
            NotificationManager::instance()
                    .showMessage("复制成功！", NotificationManager::Success, MainWindow::getInstance());
            // 取消选中状态
            model->setData(index, false, Qt::UserRole + 1);
        });
        menu->addSeparator();
    ...
    // 添加删除选项
    QIcon deleteIcon(":/resources/icon/delete.png");
    ...
    connect(deleteAction, &QAction::triggered, [index, model = index.model()]() {
        if (ChatListModel* chatModel = qobject_cast<ChatListModel*>(const_cast<QAbstractItemModel*>(model))) {
            ...
            NotificationManager::instance()
                    .showMessage("删除成功！", NotificationManager::Success, MainWindow::getInstance());
        }
    });

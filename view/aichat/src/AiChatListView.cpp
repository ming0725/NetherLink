...
AiChatListView::AiChatListView(QWidget *parent)
    : QListView(parent)
    , m_smoothScrollValue(0)
{
    ...
    connect(customScrollBar, &SmoothScrollBar::valueChanged,
            this, &AiChatListView::onCustomScrollValueChanged);
    connect(scrollAnimation, &QPropertyAnimation::finished,
            this, &AiChatListView::onAnimationFinished);
}
...
void AiChatListView::setModel(QAbstractItemModel *model)
{
    if (this->model()) {
        disconnect(this->model(), &QAbstractItemModel::rowsInserted,
                  this, &AiChatListView::onModelRowsChanged);

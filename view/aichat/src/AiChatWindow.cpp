...
void AiChatWindow::setupUI()
{
    ...
    statusIcon->setPixmap(QPixmap(":/resources/icon/online.png")
        .scaled(14, 14,
                Qt::KeepAspectRatio,
    ...
    connect(m_inputBar, &FloatingInputBar::sendText,
            this, &AiChatWindow::onSendMessage);
...
void AiChatWindow::onConnectionError(const QString& error)
{
    ...
    NotificationManager::instance()
            .showMessage(tr("连接错误: %1").arg(error),
...
void AiChatWindow::onRequestError(const QString& errorMessage)
{
    ...
    NotificationManager::instance()
            .showMessage(tr("请求错误: %1").arg(errorMessage),
                        NotificationManager::Error,
                         MainWindow::getInstance());
} 
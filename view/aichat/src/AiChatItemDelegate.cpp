    ...
    , m_contextMenu(new TransparentMenu(parent))
{
    ...
    // 创建右键菜单
    QAction* copyAction = new QAction(QIcon(":/resources/icon/copy.png"), "复制", this);
...
QSize AiChatItemDelegate::messageSizeHint(const QStyleOptionViewItem& option, const AiChatMessage* message) const
{
    ...
    if (isFromMe) {
        ...
        return QSize(option.rect.width(),
                    qMax(AVATAR_SIZE, bubbleHeight) + 2 * VERTICAL_MARGIN);
    } else {
...
bool AiChatItemDelegate::editorEvent(QEvent* event, QAbstractItemModel* model,
...
void AiChatItemDelegate::onCopyMessage()
{
    if (!m_selectedText.isEmpty()) {
        NotificationManager::instance()
                .showMessage("复制成功！", NotificationManager::Success);

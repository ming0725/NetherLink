...
QString LocalDataStore::connectionNameForPath(const QString& path) const
{
    if (path == m_globalDatabasePath) {
        return QStringLiteral("netherlink_global_store");
    }

    return QStringLiteral("netherlink_account_store_%1").arg(m_activeAccountHash);
}

QSqlDatabase LocalDataStore::databaseForPath(const QString& path)
{
    const QString name = connectionNameForPath(path);
